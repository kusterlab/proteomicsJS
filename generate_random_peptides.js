...
var x = [...Array(1000)].map(x => utility.randomizer_b(peptide, offset))
var y = uniq(x);
var modStrings = [...Array(y.length)].map(x => "");

var a = utility.create_post_body_for_prediction(y, 2, 30, modStrings);
...
def =d3v4.json('https://www.proteomicsdb.org/logic/api/getFragmentationPrediction.xsjs', {
	...
	.then(e => {
		return e.map( el => {
			...
			return stuff.ipsa_helper["comparison"]["spectral_angle"](binarySpectrum["intensity_1"], binarySpectrum["intensity_2"]);
		});
	});
