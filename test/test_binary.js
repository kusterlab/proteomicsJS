  ...
  describe('binary', () => {
    ...
    describe('binary spectra search', () => {
      ...
      it('getClosestValue', () => {
        ...
        const sorter_asc_mz = binary.my_sorter('mz', 'asc');
        var spectrum_1 = spectrum_1.sort(sorter_asc_mz);
        const a = binary.getClosestValues_spec2(spectrum_1, 10);
        compare_ppmF = compare_ppm_FACTORY('mz');
        assert.deepEqual(a,
          { mz: 9.99999999999, intensity: 4569 });
      });
      it('retrieve_merged_peaks_contain', () => {
        const spectrum_1 = [{ mz: 10.00002, intensity: 10 }, { mz: 12, intensity: 88 },
          { mz: 9, intensity: 123 }, { mz: 9.99999999999, intensity: 4569 }];
        const spectrum_2 = [{ mz: 10, intensity: 545 }];
        const contain_2_peak = binary.binary_search_spectrum(spectrum_1, spectrum_2);
        assert.deepEqual(
          contain_2_peak, [{
            id_1: [1, 2],
            id_2: 0,
      ...
      it('retrieve_merged_peaks_contain2', () => {
        const spectrum_2 = [{ mz: 10.00002, intensity: 10 }, { mz: 12, intensity: 88 },
          { mz: 9, intensity: 123 }, { mz: 9.99999999999, intensity: 4569 }];
        const spectrum_1 = [{ mz: 10, intensity: 545 }];
        const contain_1_peak = binary.binary_search_spectrum(spectrum_1, spectrum_2);
        assert.deepEqual([
          {
            id_1: [],
            id_2: 0,
      ...
      it('swapped labels exists 2x grouping -> out', () => {
        ...
        const r = binary.full_merge(spec_1_spec_2, spec_2_spec_1);
        assert.deepEqual(
